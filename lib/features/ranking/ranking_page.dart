...
import '../map/map_controller.dart'; // usa areasProvider
...
class _RankingPageState extends ConsumerState<RankingPage> {
  ...
  // Por enquanto seu app é focado em Jaraguá do Sul
  ...
  Widget _buildCityView(BuildContext context, List<AreaFeature> areas) {
    // média da cidade = média de todas as notas de todos os bairros
    ...
    return ListView(
      ...
      children: [
        ...
        _CityCard(
          cityName: _selectedCity ?? 'Jaraguá do Sul',
  ...
  Widget _buildNeighborhoodView(
    ...
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 4, 16, 24),
      children: [
        // Filtros (preço/tipo/quartos/área)
        SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          padding: const EdgeInsets.only(bottom: 8),
          child: Row(
        ...
        // Chip de cidade (filtro ativo) — agora com largura limitada
        if (_selectedCity != null)
          Padding(
            ...
            child: _FilterTagChip(
              ...
              label: _selectedCity!,
              onDeleted: () {
                // remover cidade → volta para visão de cidade
                setState(() {
                  _selectedCity = null;
                  _selectedArea = null;
  ...
  Widget _buildPropertyView(BuildContext context, List<AreaFeature> areas) {
    // Regra nova:
    // - Se tiver filtro de bairro (_selectedArea != null) → só imóveis desse bairro
    // - Se não tiver filtro → listar até 100 melhores imóveis (todas as avaliações)

    // Monta lista (area + rating) de TODO mundo
    ...
    // Aplica filtro de bairro se existir
    if (_selectedArea != null) {
      filtered = filtered.where((e) => e.area.id == _selectedArea!.id);
    }

    // Ordena por nota (desc) e corta nos 100 primeiros
    ...
    return ListView(
      ...
      children: [
        ...
        // Chips de cidade + bairro (com largura limitada)
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: [
            ...
            if (_selectedArea != null)
              _FilterTagChip(
                ...
                onDeleted: () {
                  setState(() {
                    _selectedArea = null;
                    // continua na aba Imóveis, mas sem filtro de bairro
                  });
                },
              ),
          ],
        ...
        // Lista de imóveis (1 card por avaliação)
        ...top.map(
          (entry) => _PropertyCard(
...
// ----------------------------------------
// MODELOS AUXILIARES PARA RANKING
// ----------------------------------------
...
class _PropertyEntry {
  final AreaFeature area;
  final dynamic rating; // Rating (tipo real do seu modelo)
...
/// CHIP DE TAG (cidade/bairro) — agora com largura MÁXIMA definida
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      constraints: const BoxConstraints(
        maxWidth: 260, // evita ocupar a tela inteira quando tem só 1 chip
      ),
